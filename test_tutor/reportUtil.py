...
if __name__ == '__main__':
    ...
    tutorpc={'success': ['启动', '登录', '选择直播间', '选择题开启', '懂不懂开启', '懂不懂关闭', '判断题开启', '判断题关闭', '做题开启', '做题关闭', '发积分开启', '退出'], 'error': [], 'fail': [('选择题关闭', ['执行assert_pic_exist操作失败，失败原因为==Template(d://img//choice_end.png) does not exist in screen, message: ']), ('轻选择题开启', ['执行assert_pic_exist操作失败，失败原因为==Template(d://img//lightchoice.png) does not exist in screen, message: ']), ('轻选择题关闭', ['执行assert_pic_exist操作失败，失败原因为==Template(d://img//lightchoice_end.png) does not exist in screen, message: ']), ('发积分关闭', ['执行assert_pic_exist操作失败，失败原因为==Template(d://img//point_end.png) does not exist in screen, message: '])], 'skip': []}
    case=["tutor"]
    answer={"tutor":0}
    res = result(time,fp,title="UI自动化测试报告",tutorpc=tutorpc)
    # result()
    res.generatereport()
    # print (res.chart())
